...
def tip(socket, file_path, network, logger):
    """
    ...
    if "cardano-cli" in errors.decode():
        logger.error("Node Socket Not Found; Exiting Application.")
        sys.exit(1)


def get_latest_block_number(socket, file_path, network, logger):
    # get current tip
    tip(socket, file_path, network, logger)
